...
# Leave GEMINI_MODEL empty to use the CLI's auto-selected default
...
if [[ -f "$CONSTITUTION" ]]; then
    ...
    if grep -q "Git Autonomy.*DISABLED" "$CONSTITUTION" 2>/dev/null; then
        GIT_AUTONOMY=false
    fi
fi
...
Model (default: auto-selects latest, e.g. Gemini 3 Flash):
  Override with: GEMINI_MODEL=gemini-1.5-pro ./scripts/ralph-loop-gemini.sh
...
# Build Gemini flags
# -p reads the prompt content directly (non-interactive/headless mode)
GEMINI_MODEL_FLAG=""
...
echo -e "${BLUE}Model:${NC}    ${GEMINI_MODEL:-auto-selected (Gemini 3 Flash)}"
echo -e "${BLUE}Prompt:${NC}   $PROMPT_FILE"
echo -e "${BLUE}Branch:${NC}   $CURRENT_BRANCH"
echo -e "${YELLOW}YOLO:${NC}     $([ "$YOLO_ENABLED" = true ] && echo "ENABLED" || echo "DISABLED")"
echo -e "${YELLOW}Git Push:${NC} $([ "$GIT_AUTONOMY" = true ] && echo "ENABLED" || echo "DISABLED")"
...
while true; do
    ...
    # Run Gemini CLI with prompt passed via -p flag (headless mode)
    # --yolo auto-approves all tool calls
    PROMPT_CONTENT="$(cat "$PROMPT_FILE")"
    GEMINI_OUTPUT=""
    if GEMINI_OUTPUT=$("$GEMINI_CMD" -p "$PROMPT_CONTENT" $GEMINI_YOLO_FLAG $GEMINI_MODEL_FLAG 2>&1 | tee "$LOG_FILE"); then
        if [ -n "$WATCH_PID" ]; then
